...
import { htmlContentSchema, type HTMLContentFormData } from '@/lib/schemas/html-content-schema';
...
interface HTMLContentFormProps {
  ...
  onSave: (data: HTMLContentFormData) => Promise<void>;
  isLoading?: boolean;
  title: string;
}

export function HTMLContentForm({
  initialData,
  onSave,
  isLoading = false,
  title,
  ...
  const form = useForm<HTMLContentFormData>({
  ...
  const handleSubmit = async (data: HTMLContentFormData) => {
    try {
      await onSave(data);
